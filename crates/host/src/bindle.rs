...
pub async fn get_client(
    ...
    let keyring: KeyRing = match keyring_path.load().await {
        Ok(k) => k,
        Err(e) => {
            eprintln!("WARN: Got error when trying to load keyring: {e}\n\n Attempting to fetch host keys from server");
            ...
            if let Err(e) = keyring_path.save(&k).await {
                warn!("unable to save fetched host keys to {}. Will continue with keyring in memory: {}", keyring_path.display(), e);
            }
            k
        }
    };
...
pub async fn fetch_actor(
    ...
    let parcel = match parcel.as_deref() {
        Some([parcel]) => parcel,
        _ => bail!("actor bindle should contain exactly one parcel"),
    };
